...
# Evaluate reasoning RL checkpoint on full test set — split: task
...
test_path="$HOME/scratch/raofu/3DHaptic/annotation_verl_split_task_test.json"

mkdir -p outputs/eval_qwen3vl_dapo_reasoning_split_task
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.experiment_name='eval_qwen3vl_dapo_reasoning_split_task' \
    trainer.n_gpus_per_node=1 \
    trainer.nnodes=1 \
    trainer.save_freq=20 \
    trainer.test_freq=5 \
    trainer.val_only=True \
    trainer.validation_data_dir=outputs/eval_qwen3vl_dapo_reasoning_split_task \
